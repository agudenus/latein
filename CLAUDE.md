...
- `src/dashboard/` — **M7 read-only web monitor** (`state.rs` derives one JSON snapshot from SQLite; `render.rs` server-renders it; `assets/` holds the vendored Nocturne stylesheet, the component CSS and a small vanilla poll script, all `include_str!`'d so the binary is self-contained). A **separate process** from the daemon: no shared state, `GET` routes only (`/`, `/api/state`, `/assets/*`), no button/form/POST anywhere, and a test enumerates the routes to keep it that way. Money crosses the JSON boundary as Decimal **strings**, never numbers, and is never parsed back in JS. The break state (design `2d`) takes the page over when the scanner is *blind rather than idle* — EmptyDiscovery, both transports down, all books stale, or a missing/stale `runtime_status` — and deliberately **not** for a quiet socket, a fee-model fallback, an alert cooldown or an honest REST fallback. The funnel's "survive bid–ask spread" stage renders as *not instrumented* because gaps are computed on executable sides from the start, so there is no pre-spread population to filter.
- `config/default.toml` — all knobs, including `[stream]` (enabled by default) and `[dashboard]` (loopback bind, poll cadence, evidence window); secrets only via env (`TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID`).
